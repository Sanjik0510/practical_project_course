...
from src.models.retinanet.model import RetinaNetWrapper
...
class RetinaNetTrainer:
    def __init__(self, img_dir, ann_file, epochs=20, batch_size=4, lr=0.001, momentum=0.9, weight_decay=0.0005):
        ...
        self.model = RetinaNetWrapper(num_classes=num_classes, pretrained=True).to(self.device)

    def train(self):
        print('=' * 60)
        print('Обучение RetinaNet')
        print('=' * 60)
        ...
        os.makedirs('results/retinanet/weights', exist_ok=True)
        os.makedirs('results/retinanet/logs', exist_ok=True)
        
        log_file = open('results/retinanet/logs/train_log.txt', 'w')
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == self.epochs - 1:
                torch.save(self.model.state_dict(), f'results/retinanet/weights/epoch_{epoch+1}.pth')
                print(f'   Сохранены веса за эпоху {epoch+1}')
                
        log_file.close()
        torch.save(self.model.state_dict(), 'results/retinanet/weights/best.pth')
        print('\n' + '=' * 60)
        print('Обучение RetinaNet завершено!')
        print('=' * 60)