...
from src.models.faster_rcnn.model import FasterRCNNWrapper

class FasterRCNNTrainer:
    def __init__(self, img_dir, ann_file, epochs=20, batch_size=2, lr=0.005, momentum=0.9, weight_decay=0.0005):
        ...
        self.model = FasterRCNNWrapper(num_classes=num_classes, pretrained=True).to(self.device)
    ...
    def train(self):
        ...
        os.makedirs('results/faster_rcnn/weights', exist_ok=True)
        os.makedirs('results/faster_rcnn/logs', exist_ok=True)
        
        log_file = open('results/faster_rcnn/logs/train_log.txt', 'w')
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == self.epochs - 1:
                torch.save(self.model.state_dict(), f'results/faster_rcnn/weights/epoch_{epoch+1}.pth')
                print(f'   Сохранены веса за эпоху {epoch+1}')
                
        log_file.close()
        torch.save(self.model.state_dict(), 'results/faster_rcnn/weights/best.pth')
        print('\n' + '=' * 60)
